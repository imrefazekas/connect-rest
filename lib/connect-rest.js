...
var VERSION = exports.VERSION = '1.8.5';
var logger;
...
function returnResult( err, result, res ){
	...
	if( err ){
		...
		for( key in headers )
			res.setHeader( key, headers[key] );
		res.statusCode = result.resOptions.statusCode || 200;
...
function processRequest(req, res, action, bodyObj){
	...
	action( req, bodyObj, function(err, result){
		logger.restlog( err, 'Result', { result: result }, 'verbose' );
...
exports.rester = function( options ) {
	...
	logger.restlog( null, 'connect-rest has been configured.', purify( { options: options }, { arrayMaxSize: 100, maxLevel: 3 }, 0, []), 'verbose' );
