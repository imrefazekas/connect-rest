...
function addPath(key, path, action, prototypeObject, options){
	mapping[ key ].push( new Route( path, prototypeObject, options,
		function(request, content, callback){
			if(action.length == 3) {
				action(request, content, function(err, result, statusCode){
					callback(err, { contentType: options.contentType || 'application/json', result : result, statusCode: statusCode } );
				});
			} else {
		...
		function(err, results){
			...
			if( asyncCall ){
				...
				if( err ){
					...
					res.writeHead( result.statusCode || 200, {'Content-Type': result.contentType || 'application/json'});
