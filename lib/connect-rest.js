...
var extRequest = require('./extensions/request');
...
function processRequest(req, res, matching, bodyObj){
	...
	var callChain =  _.map( matching, function(func){
		...
		return async.apply( func, {headers: headers, parameters: req.query, params:req.query}, bodyObj );
	} );
...
exports.bodyParser = function( opts ){
	var options = opts || {};
	return bodyParser.parseBody( 
		_, { sizeLimit: options.sizeLimit || LOAD_SIZE_LIMIT, rejectUnknown: options.rejectUnknown || true }
	);
};
	...
	return function(req, res, next) {
		...
		var routes = _.filter(
			mapping[ req.method ], function(route){ return route.matches(
				req, pathname, CONTEXT, req.getVersion(), _, semver, true, (API_KEYS && API_KEYS.indexOf(req.query.api_key)==-1)
