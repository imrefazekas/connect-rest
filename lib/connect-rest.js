	...
	async.series( callChain,
		function(err, results){
			...
			if( looseAfter>0 && (Date.now() - requestTime)>looseAfter ){
				logger.restlog( new Error('Request timed out'), 'Request timed out', req.headers, 'errpr' );
...
exports.checkCall = function( req, res ){
	...
	var routes = _.filter( mapping[ req.method ], function(route){
		var apikeys = route.apikeys || API_KEYS;
		return route.matches(
			req, pathname, req.headers['accept-version'] || req.headers['x-api-version'] || '*', _, semver, false, (apikeys && apikeys.indexOf( apiKey ) === -1)
...
exports.rester = function( options ) {
	...
	function perform( pathname, req, res, next ){
		...
		var routes = _.filter( mapping[ req.method ], function(route){
			var apikeys = route.apikeys || API_KEYS;
			return route.matches(
				req, pathname, req.headers['accept-version'] || req.headers['x-api-version'] || '*', _, semver, true, (apikeys && apikeys.indexOf(apiKey )=== -1)
			) && (!route.options.validator || route.options.validator(req, res) );
		} );
