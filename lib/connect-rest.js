...
exports.checkCall = function( req ){
	...
	var routes = _.filter(
		mapping[ req.method ], function(route){ return route.matches(
			req, pathname, req.headers['accept-version'] || req.headers['x-api-version'] || '*', _, semver, false, (API_KEYS && API_KEYS.indexOf(req.query.api_key)==-1)
...
exports.rester = function( options ) {
	...
	return function(req, res, next) {
		...
		var routes = _.filter(
			mapping[ req.method ], function(route){ return route.matches(
				req, pathname, req.headers['accept-version'] || req.headers['x-api-version'] || '*', _, semver, true, (API_KEYS && API_KEYS.indexOf(req.query.api_key)==-1)
			) && (!route.options.validator || route.options.validator(req, res) ); }
		);
