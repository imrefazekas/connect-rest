...
function finalResult(err, result, resOptions, contentType, callback){
	if( !result || isString(result) )
		return callback( err, result, resOptions );

	if( isReadableStream(result) ){
		var readRes = '';
		result.setEncoding( resOptions.encoding || 'utf8' );
		result.on('data', function (chunk) {
			readRes += chunk;
		});
		result.on('error', function (error) {
			return callback( error, null, resOptions );
		});
		result.on('end', function () {
			return callback( err, readRes, resOptions );
		});
	}
	else if( isBuffer(result) ){
		var realRes = (contentType === 'application/json') ? JSON.stringify( result ) : result.toString( resOptions.encoding || 'utf8' );
		return callback( err, realRes, resOptions ); 
	}
	else if( isFunction(result) ){ return result( callback ); }
		...
		function(request, content, callback){
			if(action.length === 3) {
				action(request, content, function(err, result, resOptions){
					finalResult( err, result, resOptions || {}, options.contentType, function(realErr, realResult, realOptions){
						callback(realErr, { contentType: options.contentType || 'application/json', result : realResult, resOptions: realOptions || {} } );
					} );
				});
			} else {
				var result = action(request, content);
				var err;
				finalResult( err, result, {}, options.contentType, function(realErr, realResult, realOptions){
					callback(realErr, { contentType: options.contentType || 'application/json', result : realResult, resOptions: realOptions } );
				});
			}
		}, _
