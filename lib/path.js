...
var PARAMETER_O_S_DELIMETER = '[';
var PARAMETER_O_E_DELIMETER = ']';
...
Path.prototype.matches = function( req, pathname, version, _, semver ){
	if( this.isRegex ){
		...
		if( _( ptokens[ ptokens.length-1 ] ).startsWith( PARAMETER_O_S_DELIMETER ) && utokens.length == ptokens.length-1 )
			utokens.push('');
		...
		for (i=0;i<utokens.length;i++)
			...
			else if( _( ptokens[i] ).startsWith( PARAMETER_O_S_DELIMETER ) ){
				if( utokens[i].length>0  )
					parameterReplacements[ ptokens[i].substring( PARAMETER_O_S_DELIMETER.length, ptokens[i].length-PARAMETER_O_E_DELIMETER.length ) ] = utokens[i];
			}
			else
