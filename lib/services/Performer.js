...
var util = require('../util/Converter');
...
function returnResult( err, result, res ){
	var headers = extend( {}, exports.globalHeaders );
	if( err ){
...
exports.process = function(req, res, action, bodyObj){
	...
	req.format = function(){
		var obj = { headers: self.headers, parameters: self.parameters };
...
exports.perform = function( req, res, next ){
	...
	var router = self.getRouterMatching( req, res );
	...
	if( routeMatched ){
		...
		if( routeMatched.apiKeyRequired() ){
			...
			var apikeys = routeMatched.apikeys || self.config.apikeys;
